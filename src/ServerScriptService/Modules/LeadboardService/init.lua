...
local rebirthLeadboardModule = require(script.rebirthLeadboard)
...
function LeadboardService:UpdateDataStore()
	for _, player in Players:GetPlayers() do
		...
		local rebirth = PlayerDataHandler:Get(player, "rebirth")

		if rebirth then
			rebirthLeadboardModule:RegisterEntry(player.UserId, rebirth)
		end
...
function LeadboardService:UpdateLeadboard(leadboardName, items)
	...
	for index, value in items do
		...
		pcall(function()
			...
			newItem.ImageLabel.Image = LeadboardService:GetThumb(value.playerUserId)
